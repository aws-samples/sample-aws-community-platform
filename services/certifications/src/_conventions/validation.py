...
def require_secret_str(value: Any, field: str, *, max_len: int, min_len: int = 1) -> str:
    """Like require_str but WITHOUT the angle-bracket XSS guard. For passwords
    and other secrets that are never stored, logged, or rendered: the HTML guard
    is an output-safety concern that doesn't apply, and rejecting '<'/'>' here
    breaks legitimate credentials (e.g. Secrets Manager generated passwords)."""
    require(isinstance(value, str), field, "must be a string")
    require(min_len <= len(value) <= max_len, field, f"length must be {min_len}-{max_len}")
    return value
