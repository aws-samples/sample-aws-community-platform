...
def require_https_url(value: Any, field: str, *, max_len: int) -> str:
    """Accept only an absolute https:// URL.

    require_str alone is NOT sufficient for a value that will be rendered into
    an anchor href: it rejects only '<' and '>', so `javascript:` and `data:`
    URIs pass it unchanged. React does not sanitize href, so such a value
    executes when a reviewer clicks the link. Scheme allow-listing has to
    happen here, at the write boundary.
    """
    value = require_str(value, field, max_len=max_len)
    stripped = value.strip()
    require(stripped == value, field, "must not have leading or trailing whitespace")
    # Compare lowercased: `JavaScript:` and `HtTpS://` are both valid schemes to
    # a browser, so a case-sensitive prefix test is bypassable.
    require(stripped.lower().startswith("https://"), field, "must be an https:// URL")
    # A bare scheme is not a usable link and would render as a dead anchor.
    require(len(stripped) > len("https://"), field, "must include a host")
    return stripped
